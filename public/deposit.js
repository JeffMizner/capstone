...
function DepositForm(props){
    ...
    function depositMoney() {
        ...
        if(!isNaN(deposit) && Number(deposit) > 0) {
            ...
            props.setStatus(`Successfully Deposited: ${Number(deposit).toLocaleString("en-US",{style: "currency", currency: "USD"})}!`);
            setTimeout(() => props.setStatus(''), 4000);  
            setDeposit(''); 
        } else if (Number(deposit) < 0) {
            props.setStatus(`ERROR: Deposit amount is either 0 or negative. Please try again.`);
            setTimeout(() => props.setStatus(''), 4000); 
            setDeposit(''); 
        } else {
    ...
    return(
        ...
        <input 
            ...
            {disabled ? (
                <><button type="submit" className="btn btn-dark" disabled="disabled" onClick={depositMoney}>Deposit</button></>
            ):(
                <><button type="submit" className="btn btn-dark" onClick={depositMoney}>Deposit</button></>
            )}
        </>
    );
}
...
function LoginMsg(props){
    ...
    return(
        <>
        <h4> &#9432; There is currently no user logged in. Please log in and return to this page.</h4><br/><br/>
