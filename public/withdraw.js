...
function WithdrawForm(props){
    ...
    function withdrawMoney() {
        ...
        if(!isNaN(withdraw) && Number(withdraw) > 0 && Number(balance) >= Number(withdraw)) {
            ...
            props.setStatus(`Successfully Withdrew: ${Number(withdraw).toLocaleString("en-US",{style: "currency", currency: "USD"})}!`);
            setTimeout(() => props.setStatus(''), 4000);  
            setWithdraw(''); 
        } else if (Number(withdraw) > Number(balance)) {
            props.setStatus(`ERROR: Withdraw amount is greater than the available balance. Please try again.`);
            setTimeout(() => props.setStatus(''), 4000); 
            setWithdraw(''); 
        } else if (Number(withdraw) < 0) {
            props.setStatus(`ERROR: Withdraw amount is either 0 or negative. Please try again.`);
            setTimeout(() => props.setStatus(''), 4000); 
            setWithdraw(''); 
        } else {
    ...
    return(
        ...
        <input 
            ...
            {disabled ? (
                <><button type="submit" className="btn btn-dark" disabled="disabled" onClick={withdrawMoney}>Withdraw</button></>
            ):(
                <><button type="submit" className="btn btn-dark" onClick={withdrawMoney}>Withdraw</button></>
            )}
        </>
    );
}
